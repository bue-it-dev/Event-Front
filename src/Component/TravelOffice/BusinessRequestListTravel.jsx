...
import TravelOfficeTabs from "./TravelOfficeTabs";
import TravelOffice from "./TravelOffice";

const BusinessRequestListTravel = () => {
  ...
  const GetBusinessRequest = async () => {
    try {
      const response = await axios.get(
        `${URL.BASE_URL}/api/BusinessRequest/GetTravelBusinessRequest`,
        {
          headers: {
    ...
    rows: businessRequest.map((data, i) => ({
      ...
      action: (
        <div className="d-flex justify-content-around">
          <Link
            to={{
              pathname: "/business-request-details-travel-office",
...
export default BusinessRequestListTravel;
