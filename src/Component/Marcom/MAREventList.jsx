...
import BudgetOfficeTabs from "./MARCOMTabs";
  ...
  const GetEvents = async (empID) => {
    try {
      const response = await axios.get(
        `${URL.BASE_URL}/api/EventEntity/get-eventRequestMarcom/`,
        {
          headers: {
    ...
    rows: events.map((event, i) => ({
      ...
      actions: (
        <>
          <Link
            to={{
              pathname: "/event-request-details-marcom",
              state: {
