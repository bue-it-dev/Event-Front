  ...
  const GetEvents = async (empID) => {
    try {
      const response = await axios.get(
        `${URL.BASE_URL}/api/EventEntity/get-eventRequestEventOffice/`,
        {
          headers: {
    ...
    rows: events.map((event, i) => ({
      ...
      actions: (
        <>
          <Link
            to={{
              pathname: "/event-details-event-office",
              state: {
...
export default EventOfficeRequestList;
