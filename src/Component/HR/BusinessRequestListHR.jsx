...
import VCBTabs from "./HRTabs";

const BusinessRequestListHR = () => {
  ...
  const GetBusinessRequest = async () => {
    try {
      const response = await axios.get(
        `${URL.BASE_URL}/api/BusinessRequest/GetBusinessRequestThatHasAllowance`,
        {
          headers: {
      ...
      setError("Failed to fetch business requests. Please try again later.");
    } finally {
      setisLoading(false);
    }
  };
    ...
    rows: businessRequest.map((data, i) => ({
      ...
      action: (
        <div className="d-flex justify-content-around">
          <Link
            to={{
              pathname: "/business-request-details-hr",
...
export default BusinessRequestListHR;
